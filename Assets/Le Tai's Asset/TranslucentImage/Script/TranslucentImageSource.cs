...
#region Private Field
    [SerializeField]
    BlurAlgorithmType blurAlgorithmSelection = BlurAlgorithmType.ScalableBlur;
    ...
    private void OnRenderImage(RenderTexture source, RenderTexture destination)
    {
        ...
        if (blurAlgorithm != null && BlurConfig != null)
        {
            if (shouldUpdateBlur())
            {
                ...
                blurAlgorithm.Blur(cmd, source, BlurRegion, BackgroundFill, blurredScreen);
            }

            if (Preview)
            {
                previewMaterial.SetVector(ShaderId.CROP_REGION, BlurRegion.ToMinMaxVector());
                cmd.Blit(BlurredScreen, destination, previewMaterial);
            }
        }
